...
$(document).ready(function(){
    ...
    function calc_gearbox(){
        ...
        gear1options = gear1options.filter(function(gear){
            ...
            if ($("input#min1diam").val() && gear.last()/20+0.1 < $("input#min1diam").val()/$("select#min1diam-u").val()) return false;
            if ($("input#max1diam").val() && gear.last()/20+0.1 > $("input#max1diam").val()/$("select#max1diam-u").val()) return false;
            return true;
        });
        ...
        gear4options = gear4options.filter(function(gear){
            ...
            if ($("input#min4diam").val() && gear.last()/20+0.1 < $("input#min4diam").val()/$("select#min4diam-u").val()) return false;
            if ($("input#max4diam").val() && gear.last()/20+0.1 > $("input#max4diam").val()/$("select#max4diam-u").val()) return false;
            return true;
        });
        if (two_stage){
            ...
            gear2options = gear2options.filter(function(gear){
                ...
                if ($("input#min2diam").val() && gear.last()/20+0.1 < $("input#min2diam").val()/$("select#min2diam-u").val()) return false;
                if ($("input#max2diam").val() && gear.last()/20+0.1 > $("input#max2diam").val()/$("select#max2diam-u").val()) return false;
                return true;
            });
            ...
            gear3options = gear3options.filter(function(gear){
                ...
                if ($("input#min3diam").val() && gear.last()/20+0.1 < $("input#min3diam").val()/$("select#min3diam-u").val()) return false;
                if ($("input#max3diam").val() && gear.last()/20+0.1 > $("input#max3diam").val()/$("select#max3diam-u").val()) return false;
                return true;
            });
        }
                ...
                if (two_stage){
                    gear2options.forEach(gear2 => {
                        gear3options.forEach(gear3 => {
                            ...
                            if ($("input#min12dist").val() && (gear1.last()+gear2.last())/40 < $("input#min12dist").val()/$("select#min12dist-u").val()) return;
                            if ($("input#max12dist").val() && (gear1.last()+gear2.last())/40 > $("input#max12dist").val()/$("select#min12dist-u").val()) return;
                            if ($("input#min34dist").val() && (gear3.last()+gear4.last())/40 < $("input#min34dist").val()/$("select#min34dist-u").val()) return;
                            if ($("input#max34dist").val() && (gear3.last()+gear4.last())/40 > $("input#max34dist").val()/$("select#min34dist-u").val()) return;
                            if ($("select#clearance1").val() != "None" && (gear1.last()+gear2.last()-(gear3.last()+2))/40 < ($("select#clearance1").val() == "Custom" ? 
                                $("input#clearance1custom").val()/$("select#clearance1custom-u").val() : $("select#type1").val())) return;
                            if ($("select#clearance4").val() != "None" && (gear3.last()+gear4.last()-(gear2.last()+2))/40 < ($("select#clearance4").val() == "Custom" ? 
                                $("input#clearance4custom").val()/$("select#clearance4custom-u").val() : $("select#type4").val())) return;
                    ...
                    if (ratio < min_ratio || ratio > max_ratio) return;
                    if ($("input#min14dist").val() && (gear1.last()+gear4.last())/40 < $("input#min14dist").val()/$("select#min14dist-u").val()) return;
                    if ($("input#max14dist").val() && (gear1.last()+gear4.last())/40 > $("input#max14dist").val()/$("select#min14dist-u").val()) return;
